...
BACKGROUND_IMAGE = os.path.join(HERE, "background.jpg")  # fallback to background.jpeg if .jpg not present
    ...
    for i in range(1, n + 1):
        ...
        txt = (
            TextClip(
                ...
                color=TEXT_COLOR,
                method="caption",
                size=(text_width, TEXT_AREA_HEIGHT),
                align="center",
...
def main():
    ...
    for idx, block in enumerate(TEXT_BLOCKS, start=1):
        ...
        try:
            ...
            fallback = fallback.set_start(cursor).set_duration(seg_duration).set_position(("center", TEXT_Y))
            clips.append(fallback)
        finally:
            gc.collect()

        # Add a post-text pause freezing the last typing frame to guarantee identical metrics/position
        try:
            last_frame = seg_clip.get_frame(seg_duration - 1e-3)
            pause_clip = ImageClip(last_frame).set_start(cursor + seg_duration).set_duration(PAUSE_AFTER).set_position(("center", TEXT_Y))
            clips.append(pause_clip)
        except Exception:
            # Fallback: keep previous static TextClip approach
            pause_clip = TextClip(
                seg_text,
                fontsize=FONT_SIZE,
                font=FONT,
                ...
                stroke_color=STROKE_COLOR,
                stroke_width=STROKE_WIDTH,
            ).set_start(cursor + seg_duration).set_duration(PAUSE_AFTER).set_position(("center", TEXT_Y))
            clips.append(pause_clip)
