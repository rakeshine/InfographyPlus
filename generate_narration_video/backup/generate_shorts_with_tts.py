...
TEXT_BLOCKS = [
    {"text": "German Car Company hiring in Mahindra City, Chennai.", "duration": 4.0},
    {"text": "Work: 5 days a week. Saturday & Sunday off.", "duration": 4.0},
    {"text": "Eligibility: Male & Female, Age 18–23, Freshers & Experienced.", "duration": 4.2},
    {"text": "Qualifications: Diploma / ITI / Any Degree passout.", "duration": 4.0},
    {"text": "Roles: Assembly Operator – 25 openings (Diploma only).", "duration": 4.2},
    {"text": "Logistics Dept – 15 openings (Degree & Diploma).", "duration": 4.0},
    {"text": "Salary: Diploma – ₹19,800 + OT ₹176/hr.", "duration": 4.0},
    {"text": "Shift: First shift only. Food provided.", "duration": 4.0},
    {"text": "Room facility & bus routes from Tambaram to Chengalpattu.", "duration": 4.2},
    {"text": "Interview Contact: 7769003348 / 9342251196 / 7769003319.", "duration": 4.2}
]
...
def main():
    # Prepare narration audio per block (TTS) and compute aligned durations
    speech_clips = []
    cursor = 0.0
    for idx, block in enumerate(TEXT_BLOCKS, start=1):
        seg_text = block["text"].strip()
        tts_path = os.path.join(OUTPUT_AUDIO_DIR, f"seg_{idx:02}.mp3")
        # Generate and cache
        if not os.path.exists(tts_path) or os.path.getsize(tts_path) == 0:
            try:
                ...
                tts.save(tts_path)
                if VERBOSE:
                    print(f"[audio] TTS generated {tts_path}")
            except Exception as e:
                print(f"[audio] TTS failed for segment {idx}: {e}")
        # Load and compute duration
        try:
            aclip = AudioFileClip(tts_path)
            seg_duration = max(0.2, float(aclip.duration))
        except Exception as e:
            print(f"[audio] Failed to load TTS for segment {idx}: {e}")
            aclip = None
            seg_duration = max(0.2, float(block.get("duration", 2.0)))
    ...
    total_duration = cursor if speech_clips else (sum(b["duration"] for b in TEXT_BLOCKS) + len(TEXT_BLOCKS) * PAUSE_AFTER)
    ...
    if image_mode:
        cursor_time = 0.0
        for idx, seg in enumerate(speech_clips, start=1):
            seg_text = seg["text"]
    ...
    # Otherwise, render the full video with narration and BGM
    final = CompositeVideoClip([bg_clip] + overlay_clips + clips, size=VIDEO_SIZE)

    # Build composite audio: speech + soft synthesized BGM
    speech_audio_tracks = []
    for seg in speech_clips:
        if seg["audio"] is not None:
            speech_audio_tracks.append(seg["audio"].set_start(seg["start"]))

    # Create a simple safe BGM (original, royalty-free) using synthesized tones
    def _bgm_wave(t):
        # t may be a scalar or a numpy array (vectorized sampling)
        tt = np.asarray(t)
        f1, f2 = 220.0, 440.0
        env = 0.3 + 0.2 * np.sin(2 * np.pi * 0.1 * tt)
        mono = 0.02 * env * (np.sin(2 * np.pi * f1 * tt) + 0.5 * np.sin(2 * np.pi * f2 * tt))
        # Ensure stereo shape: (n, 2) for arrays, (2,) for scalars
        if mono.ndim == 0:
            return np.array([mono, mono], dtype=float)
        else:
            return np.column_stack((mono, mono))

    from moviepy.audio.AudioClip import AudioClip as _AudioClip
    bgm = _AudioClip(lambda t: _bgm_wave(t), duration=total_duration).set_fps(44100)
    try:
        bgm = bgm.volumex(0.2)
    except Exception:
        pass

    combined_audio = CompositeAudioClip(speech_audio_tracks + [bgm]) if speech_audio_tracks else bgm
