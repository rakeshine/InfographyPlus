    ...
    for i, text_elem in enumerate(text_elements):
        ...
        if tspans:
            ...
            for block in combined_blocks:
                # Check if the block is a combined block and if its position matches
                # the position of the text element
                if (block['id'].startswith('combined') and 
                    abs(block['x'] - base_x) < 1.0 and 
            ...
            for block in combined_blocks:
                if (block['id'].startswith('combined') and 
                    block['text'].strip() == text_content.strip()):
                    matching_combined_block = block
                    break
