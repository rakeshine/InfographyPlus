...
def create_dialogue_box(text, cartoon_path=None):
    ...
    # Dialogue box: Right side (80%) with rounded corners on all sides
    ...
    # Add text inside the right area
    ...
    # Convert to base video clip
    base_clip = ImageClip(np.array(img)).set_duration(dialogue_duration)

    # Add cartoon GIF or image in the left 20% space
    if cartoon_path:
